...
unsigned long myChannelNumber = 000000000;	// change this to your channel number
const char * myWriteAPIKey = "XXXXXY"; // ThingSpeak channel write API key
const char * myReadAPIKey = "XXXXXZ"; // ThingSpeak channel read API key
const char * Xenon1 = "123abc";
const char * Xenon2 = "789ghi";
const char * Argon = "456def";
...
//ex: printed event=temp data=23.2
    ...
    if(strstr(event, Xenon1) != NULL){
      Particle.publish("Found: ", Xenon1,  PRIVATE);
      ...
      } else if (temp_mesh > high_temp_x1){
...
//        Serial.printf("No change X1");
        Particle.publish("No change X1");
      }
    
    }
    if(strstr(event, Xenon2) != NULL){
      Particle.publish("Found: ", Xenon2,  PRIVATE);
